...
class Scratchpad[T <: Data: Arithmetic](
  ...
  lazy val module = new LazyModuleImp(this) with HasCoreParameters {
    ...
    // val dmardata_valid = WireInit(false.B)

    when (req.write && ShiftRegister(dmaren, mem_pipeline+1)) { // dmardata_valid) {
    ...
    for (i <- 0 until nBanks) {
      ...
      when (req.spbank === i.U && !req.is_acc) {
        ...
        // dmardata_valid := bank.io.read.valid
      }
...

    {
      ...
      when(req.is_acc) {
        ...
        // dmardata_valid := accumulator.io.read.valid
      }
